  ...
  <div class="map-container" :class="containerClass">
    ...
    >
      ...
      >
        ...
        />
        <LPopup class="popup" style="margin: 0" :options="{ autoPan: false }">
...
type LeafletMarkerComponent = ComponentPublicInstance & {
  leafletObject?: { openPopup?: () => void };
};

const markerRefs = new Map<string, LeafletMarkerComponent>();
function setMarkerRef(id: string) {
  return (element: Element | ComponentPublicInstance | null) => {
    if (element && !(element instanceof Element)) {
      markerRefs.set(id, element as LeafletMarkerComponent);
    } else {
  ...
  if (leafletMarker && m) {
    nextTick(() => {
      try {
        ...
        // Popup may fail to open if marker is not yet rendered
      }
    });
  }
}
...
function nudgeIntoSafeBox() {
  const m = map.value?.leafletObject;
  if (!m || !followId.value || userPanned.value || isZooming.value || isCentering.value) return;
...
const geoJsonOptions = {
  ...
  onEachFeature: (_feature: unknown, layer: LeafletLayer) => {
    layer.on('add', () => {
